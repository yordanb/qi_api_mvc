...
async function getAcvhIpeakPlt2() {
  try {
    const [rows] = await pool.execute('SELECT ROUND(COALESCE((SUM(subquery.Jml_ipeak) / 27.0) * 100, 0), 1) AS value, "Lighting" AS label FROM (select Count(*) as Jml_ipeak from (SELECT tb_manpower_new.NRP as mp_nrp, tb_manpower_new.Nama as mp_nama, tb_manpower_new.Section as mp_section, COUNT(tb_ipeak.Judul) as jumlah_judul, SUM(tb_ipeak.Poin) as total_poin FROM db_qiagent.tb_manpower_new INNER JOIN db_qiagent.tb_ipeak USING (NRP) WHERE Crew = "Staff SSE Lighting" OR Crew = "Mekanik Lighting" GROUP BY tb_manpower_new.Nama, tb_manpower_new.NRP, tb_manpower_new.Section) as sub) AS subquery UNION SELECT ROUND(COALESCE((SUM(subquery.Jml_ipeak) / 52.0) * 100, 0), 1) AS value, "PCH" AS label FROM (select Count(*) as Jml_ipeak from (SELECT tb_manpower_new.NRP as mp_nrp, tb_manpower_new.Nama as mp_nama, tb_manpower_new.Section as mp_section, COUNT(tb_ipeak.Judul) as jumlah_judul, SUM(tb_ipeak.Poin) as total_poin FROM db_qiagent.tb_manpower_new INNER JOIN db_qiagent.tb_ipeak USING (NRP) WHERE Crew = "Staff PCH" OR Crew = "Mekanik PCH" GROUP BY tb_manpower_new.Nama, tb_manpower_new.NRP, tb_manpower_new.Section) as sub) AS subquery UNION SELECT ROUND(COALESCE((SUM(subquery.Jml_ipeak) / 38.0) * 100, 0), 1) AS value, "Mobile" AS label FROM (select Count(*) as Jml_ipeak from (SELECT tb_manpower_new.NRP as mp_nrp, tb_manpower_new.Nama as mp_nama, tb_manpower_new.Section as mp_section, COUNT(tb_ipeak.Judul) as jumlah_judul, SUM(tb_ipeak.Poin) as total_poin FROM db_qiagent.tb_manpower_new INNER JOIN db_qiagent.tb_ipeak USING (NRP) WHERE Crew = "Staff SSE Mobile" OR Crew = "Mekanik Mobile" GROUP BY tb_manpower_new.Nama, tb_manpower_new.NRP, tb_manpower_new.Section) as sub) AS subquery UNION SELECT ROUND(COALESCE((SUM(subquery.Jml_ipeak) / 69.0) * 100, 0), 1) AS value, "Pumping" AS label FROM (select Count(*) as Jml_ipeak from (SELECT tb_manpower_new.NRP as mp_nrp, tb_manpower_new.Nama as mp_nama, tb_manpower_new.Section as mp_section, COUNT(tb_ipeak.Judul) as jumlah_judul, SUM(tb_ipeak.Poin) as total_poin FROM db_qiagent.tb_manpower_new INNER JOIN db_qiagent.tb_ipeak USING (NRP) WHERE Crew = "Staff SSE Pumping" OR Crew = "Mekanik Pumping" GROUP BY tb_manpower_new.Nama, tb_manpower_new.NRP, tb_manpower_new.Section) as sub) AS subquery UNION SELECT ROUND(COALESCE((SUM(subquery.Jml_ipeak) / 39.0) * 100, 0), 1) AS value, "Big Wheel" AS label FROM (select Count(*) as Jml_ipeak from (SELECT tb_manpower_new.NRP as mp_nrp, tb_manpower_new.Nama as mp_nama, tb_manpower_new.Section as mp_section, COUNT(tb_ipeak.Judul) as jumlah_judul, SUM(tb_ipeak.Poin) as total_poin FROM db_qiagent.tb_manpower_new INNER JOIN db_qiagent.tb_ipeak USING (NRP) WHERE Crew = "Staff Big Wheel" OR Crew = "Mekanik Big Wheel" GROUP BY tb_manpower_new.Nama, tb_manpower_new.NRP, tb_manpower_new.Section) as sub) AS subquery UNION SELECT ROUND(COALESCE((SUM(subquery.Jml_ipeak) / 7.0) * 100, 0), 1) AS value, "LCE" AS label FROM (select Count(*) as Jml_ipeak from (SELECT tb_manpower_new.NRP as mp_nrp, tb_manpower_new.Nama as mp_nama, tb_manpower_new.Section as mp_section, COUNT(tb_ipeak.Judul) as jumlah_judul, SUM(tb_ipeak.Poin) as total_poin FROM db_qiagent.tb_manpower_new INNER JOIN db_qiagent.tb_ipeak USING (NRP) WHERE Crew = "Staff LCE" GROUP BY tb_manpower_new.Nama, tb_manpower_new.NRP, tb_manpower_new.Section) as sub) AS subquery UNION SELECT ROUND(COALESCE((SUM(subquery.Jml_ipeak) / 5.0) * 100, 0), 1) AS value, "MTE" AS label FROM (select Count(*) as Jml_ipeak from (SELECT tb_manpower_new.NRP as mp_nrp, tb_manpower_new.Nama as mp_nama, tb_manpower_new.Section as mp_section, COUNT(tb_ipeak.Judul) as jumlah_judul, SUM(tb_ipeak.Poin) as total_poin FROM db_qiagent.tb_manpower_new INNER JOIN db_qiagent.tb_ipeak USING (NRP) WHERE Crew = "Staff TERE" GROUP BY tb_manpower_new.Nama, tb_manpower_new.NRP, tb_manpower_new.Section) as sub) AS subquery UNION SELECT ROUND(COALESCE((SUM(subquery.Jml_ipeak) / 2.0) * 100, 0), 1) AS value, "PSC" AS label FROM (select Count(*) as Jml_ipeak from (SELECT tb_manpower_new.NRP as mp_nrp, tb_manpower_new.Nama as mp_nama, tb_manpower_new.Section as mp_section, COUNT(tb_ipeak.Judul) as jumlah_judul, SUM(tb_ipeak.Poin) as total_poin FROM db_qiagent.tb_manpower_new INNER JOIN db_qiagent.tb_ipeak USING (NRP) WHERE Crew = "PSC" GROUP BY tb_manpower_new.Nama, tb_manpower_new.NRP, tb_manpower_new.Section) as sub) AS subquery ORDER BY value DESC;');
    const lastUpdate = await getLastUpdate();
